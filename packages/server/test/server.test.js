...
function createResponseLike(result) {
  return {
    ...result,
    headers: {
      get(name) {
...
test('middleware supports verify, gate, step-up, and expiry downgrade', async () => {
  ...
  const anonymousSession = createResponseLike(await runtime.request({
    path: '/session',
  }));
  assert.equal(anonymousSession.status, 200);
  const anonymousPayload = anonymousSession.json();
  assert.equal(anonymousPayload.trust.state, 'anonymous');

  const unauthenticatedDelete = createResponseLike(await runtime.request({
    method: 'POST',
    path: '/dangerous',
    headers: sessionHeaders('user-jun'),
  }));
  assert.equal(unauthenticatedDelete.status, 403);
  const unauthenticatedDeletePayload = unauthenticatedDelete.json();
  assert.equal(unauthenticatedDeletePayload.requiredTrust, 'verified_action');

  const badChainChallenge = createResponseLike(await runtime.request({
    method: 'POST',
  ...
  const badChainPayload = badChainChallenge.json();
  assert.equal(badChainPayload.error.code, 'chain_mismatch');

  const challengeResponse = createResponseLike(await runtime.request({
    method: 'POST',
    path: '/wallet/challenge',
    headers: sessionHeaders('user-jun'),
    body: {
  ...
  const notesResponse = createResponseLike(await runtime.request({
    method: 'POST',
    path: '/notes',
    headers: sessionHeaders('user-jun', sessionId),
  }));
  ...
  const blockedDangerousResponse = createResponseLike(await runtime.request({
    method: 'POST',
    path: '/dangerous',
    headers: sessionHeaders('user-jun', sessionId),
  }));
  assert.equal(blockedDangerousResponse.status, 403);
  const blockedDangerousPayload = blockedDangerousResponse.json();
  assert.equal(blockedDangerousPayload.requiredTrust, 'verified_action');

  const stepUpChallengeResponse = createResponseLike(await runtime.request({
    method: 'POST',
    path: '/wallet/challenge',
    headers: sessionHeaders('user-jun', sessionId),
    body: {
  ...
  const stepUpVerifyResponse = createResponseLike(await runtime.request({
    method: 'POST',
    path: '/wallet/verify',
    headers: sessionHeaders('user-jun', sessionId),
    body: {
  ...
  const stepUpVerifyPayload = stepUpVerifyResponse.json();
  ...
  const allowedDangerousResponse = createResponseLike(await runtime.request({
    method: 'POST',
    path: '/dangerous',
    headers: sessionHeaders('user-jun', sessionId),
  }));
  ...
  const expiredSessionResponse = createResponseLike(await runtime.request({
    path: '/session',
    headers: sessionHeaders('user-jun', sessionId),
  }));
  assert.equal(expiredSessionResponse.status, 200);
  const expiredSessionPayload = expiredSessionResponse.json();
  assert.equal(expiredSessionPayload.trust.state, 'verified_identity');

  const expiredDangerousResponse = createResponseLike(await runtime.request({
    method: 'POST',
    path: '/dangerous',
    headers: sessionHeaders('user-jun', sessionId),
  }));
